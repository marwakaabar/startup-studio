...
const form = useForm({
  ...
  specialty: props.user.mentor?.specialty || '',
  ...
  image_url: props.user.investisseur?.profile_image || props.user.mentor?.profile_image || '',
...
<template>
  <Container class="mb-6">
    ...
    <form @submit.prevent="form.post(route('profile.info'), { method: 'patch' })" class="space-y-6" enctype="multipart/form-data">
      ...
      <!-- Pour investisseur et mentor, gestion de l'image de profil -->
      <div v-if="user.role === 'investisseur' || user.role === 'mentor'">
